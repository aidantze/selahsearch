...
//             return res.status(400).json({ error: "Missing required parameters (book, startChapter, startVerse)." });
//         }

//         // 1. Extraction (Throws errors for invalid books/bounds)
//         const passageText = extractPassage(
//             formatBookName(book),
//             Number(startChapter),
//             Number(startVerse),
//             Number(endChapter || startChapter),
//             Number(endVerse || startVerse)
//         );
...
//         pyProcess.stdin.write(JSON.stringify({ passage: passageText, songs: songs }));
//         pyProcess.stdin.end();
...
//                 res.json({
//                     search_query: {
//                         book: formatBookName(book.charAt(0).toUpperCase() + book.slice(1)),
...
//         const msg = error.message;
//         let statusCode = 400; // Default to Bad Request (Logical errors)

//         // Switch to 404 if the resource itself is missing
//         if (msg.includes("does not exist") || msg.includes("out of bounds")) {
//             statusCode = 404;
//         }
...
// app.get('/songs/matches', async (req, res) => {
//     try {
//         // 1. Extract and cast query parameters
//         console.log("...Processing parameters...");
//         const { book, startChapter, startVerse, endChapter, endVerse } = req.query;
//         if (!book || !startChapter || !startVerse) {
//             return res.status(400).json({ error: "Missing required bible coordinates." });
//         }

//         // 2. Fetch data via JS extraction (File system now, MongoDB later)
//         console.log("...Extracting bible passage...");
//         const passageText = extractPassage(
//             book,
//             Number(startChapter),
//             Number(startVerse),
...
//         console.log("...Gathering song lyrics...");
//         const songs = getAllLyrics().map(s => ({ name: s.songName, lyrics: s.lyrics }));

//         // 3. Hand off the heavy math to Python
...
//             const results = JSON.parse(pythonData);
//             res.json({
//                 search_query: { book, startChapter, startVerse, endChapter, endVerse, passageText },
//                 total_matches: results.length,
//                 matches: results
//             });
//         });
...
//     } catch (error) {
//         const msg = error.message;
//         let statusCode = 400; // Default to Bad Request (validation errors)

//         // Switch to 404 if the resource itself wasn't found
//         if (msg.includes("does not exist") || msg.includes("out of bounds")) {
//             statusCode = 404;
//         }
